    ...
    public partial class Socket_SendForm : Form
    {
        ...
        private void InitHexBox()
        {
            try
            {
                ...
                Utf8ByteCharConverter utf8Converter = new Utf8ByteCharConverter();
                tscbEncoding.Items.Add(defConverter);
                tscbEncoding.Items.Add(ebcdicConverter);
                tscbEncoding.Items.Add(utf8Converter);
        ...
        private void cmsHexBox_Opening(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                this.cmsHexBox_Paste.Enabled = this.hbPacketData.CanPaste();
                this.cmsHexBox_PasteHex.Enabled = this.hbPacketData.CanPasteHex();
            }
            catch (Exception ex)
            {
                Socket_Operation.DoLog(MethodBase.GetCurrentMethod().Name, ex.Message);
        ...
        private void cmsHexBox_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            try
            {
                ...
                {  
                    ...
                    case "cmsHexBox_Paste":

                        if (this.hbPacketData.CanPaste())
                        {
                    ...
                    case "cmsHexBox_PasteHex":

                        if (this.hbPacketData.CanPasteHex())
                        {
        ...
        private void HexBox_LinePositionChanged()
        {
            try
            {
                ...
                string sLEB128_Value = string.Empty;
                string sSLEB128_Value = string.Empty;
                ...
                if (hbPacketData.ByteProvider != null && hbPacketData.ByteProvider.Length > hbPacketData.SelectionStart)
                {
                    ...
                    if (bitInfo != null)
                    {
                        ...
                        // LEB128 / SLEB128 解码（从选中位置开始，最多10字节）
                        long lebStart = hbPacketData.SelectionStart;
                        long lebLast = hbPacketData.ByteProvider.Length;
                        int maxLebBytes = (int)Math.Min(10, lebLast - lebStart);
                        if (maxLebBytes > 0)
                        {
                            byte[] lebBuffer = new byte[maxLebBytes];
                            for (int i = 0; i < maxLebBytes; i++)
                            {
                                lebBuffer[i] = hbPacketData.ByteProvider.ReadByte(lebStart + i);
                            }
                            sLEB128_Value = DecodeLEB128(lebBuffer);
                            sSLEB128_Value = DecodeSLEB128(lebBuffer);
                        }
                    }
                }
                ...
                this.lLEB128_Value.Text = sLEB128_Value;
                this.lSLEB128_Value.Text = sSLEB128_Value;
            }
            catch (Exception ex)
            {
                Socket_Operation.DoLog(MethodBase.GetCurrentMethod().Name, ex.Message);
        ...
        private void Socket_SendForm_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        #region//LEB128 / SLEB128 解码

        /// <summary>
        /// 解码无符号 LEB128（Little Endian Base 128）
        /// </summary>
        private string DecodeLEB128(byte[] data)
        {
            try
            {
                ulong result = 0;
        ...
        /// <summary>
        /// 解码有符号 SLEB128（Signed Little Endian Base 128）
        /// </summary>
        private string DecodeSLEB128(byte[] data)
        {
            try
            {
                long result = 0;
        ...
        /// <summary>
        /// UTF-8编码的字节字符转换器，用于HexBox显示UTF-8编码的字节
        /// </summary>
        private class Utf8ByteCharConverter : IByteCharConverter
        {
            public char ToChar(byte b)
            {
                // 尝试将单个字节作为UTF-8字符显示
                char[] chars = Encoding.UTF8.GetChars(new byte[] { b });
                if (chars.Length > 0 && chars[0] != '\0' && !char.IsControl(chars[0]))
                {
                    return chars[0];
                }
                return '.';
