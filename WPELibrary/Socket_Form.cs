...
namespace WPELibrary
{
    public partial class Socket_Form : Form
    {
        ...
        public Action OnFormClosingAction { get; set; }
        private readonly ToolTip tt = new ToolTip();
        private readonly WinSockHook ws = new WinSockHook();

        private RadioButton rbFilterText;
        private RadioButton rbFilterHex;
        private TextBox txtFilter;
        private Be.Windows.Forms.HexBox hexFilter;
        ...
        private void Socket_Form_Load(object sender, EventArgs e)
        {
            ...
            Socket_Cache.System.OnHotKeyRefresh = () => this.RefreshHotKeys();
        ...
        private void Socket_Form_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            this.ExitMainForm();
            this.OnFormClosingAction?.Invoke();
        }
        ...
        private void RefreshHotKeys()
        {
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(ssi.HotKey))
                    {
                        int hotkeyID = nextID++;
                        Guid sid = ssi.SID;

                        if (WPELibrary.Lib.UserControl.HotkeyTextBox.RegisterHotkeyText(this.Handle, hotkeyID, ssi.HotKey))
                        {
                            this.hotkeyActions[hotkeyID] = () => Socket_Cache.Send.DoSend(sid);
                        }
                    }
                }
                ...
                {
                    if (!string.IsNullOrEmpty(sri.HotKey))
                    {
                        int hotkeyID = nextID++;
                        Guid rid = sri.RID;

                        if (WPELibrary.Lib.UserControl.HotkeyTextBox.RegisterHotkeyText(this.Handle, hotkeyID, sri.HotKey))
                        {
                            this.hotkeyActions[hotkeyID] = () => Socket_Cache.Robot.DoRobot(rid, null);
                        }
                    }
                }
            }
        ...
        private void UnregisterAllHotKeys()
        {
            try
            {
                foreach (int hotkeyID in this.hotkeyActions.Keys)
                {
                    User32.UnregisterHotKey(this.Handle, hotkeyID);
                }
        ...
        protected override void WndProc(ref Message m)
        {
            try
            {
                if (m.Msg == User32.WM_HOTKEY)
                {
                    ...
                    if (this.hotkeyActions.TryGetValue(hotkeyID, out Action action))
                    {
                        action();
                    }
                }
        ...
        private void InitFilterPanel()
        {
            try
            {
                this.tcSocketInfo_HotKey.Controls.Clear();

                TableLayoutPanel tlpFilter = new TableLayoutPanel();
                tlpFilter.Dock = DockStyle.Fill;
                tlpFilter.ColumnCount = 1;
                tlpFilter.RowCount = 3;
                tlpFilter.Padding = new Padding(12);
                tlpFilter.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));
                tlpFilter.RowStyles.Add(new RowStyle(SizeType.AutoSize));
                tlpFilter.RowStyles.Add(new RowStyle(SizeType.Percent, 100));
                tlpFilter.RowStyles.Add(new RowStyle(SizeType.AutoSize));

                rbFilterText = new RadioButton();
                rbFilterText.Text = "文本";
                rbFilterText.Checked = true;
                rbFilterText.AutoSize = true;
                rbFilterText.CheckedChanged += (s, e) => FilterTypeChanged();

                rbFilterHex = new RadioButton();
                rbFilterHex.Text = "十六进制";
                rbFilterHex.AutoSize = true;
                rbFilterHex.CheckedChanged += (s, e) => FilterTypeChanged();

                FlowLayoutPanel flpType = new FlowLayoutPanel();
                flpType.AutoSize = true;
                flpType.Dock = DockStyle.Fill;
                flpType.Controls.Add(rbFilterText);
                flpType.Controls.Add(rbFilterHex);

                txtFilter = new TextBox();
                txtFilter.Dock = DockStyle.Fill;
                txtFilter.Multiline = true;

                hexFilter = new Be.Windows.Forms.HexBox();
                hexFilter.Dock = DockStyle.Fill;
                hexFilter.ByteProvider = new DynamicByteProvider(new byte[0]);

                bFilter = new Button();
                bFilter.Text = "筛选";
                bFilter.AutoSize = true;
                bFilter.Click += bFilter_Click;

                bClearFilter = new Button();
                bClearFilter.Text = "清除";
                bClearFilter.AutoSize = true;
                bClearFilter.Click += bClearFilter_Click;
                ...
                flpButton.Controls.Add(bFilter);
                flpButton.Controls.Add(bClearFilter);

                tlpFilter.Controls.Add(flpType, 0, 0);
                tlpFilter.Controls.Add(txtFilter, 0, 1);
                tlpFilter.Controls.Add(hexFilter, 0, 1);
                tlpFilter.Controls.Add(flpButton, 0, 2);

                this.tcSocketInfo_HotKey.Controls.Add(tlpFilter);
        ...
        private void FilterTypeChanged()
        {
            try
            {
                if (rbFilterText != null && rbFilterHex != null)
                {
                    txtFilter.Visible = rbFilterText.Checked;
                    hexFilter.Visible = rbFilterHex.Checked;
                }
            }
            catch (Exception ex)
            {
        ...
        private void bFilter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rbFilterText.Checked)
                {
                    if (!string.IsNullOrEmpty(txtFilter.Text))
                    {
                        searchData = Socket_Operation.StringToBytes(Socket_Cache.SocketPacket.EncodingFormat.GBK, txtFilter.Text);
                    }
                }
                else if (rbFilterHex.Checked)
                {
                    DynamicByteProvider dbp = hexFilter.ByteProvider as DynamicByteProvider;
                    if (dbp != null && dbp.Bytes.Count > 0)
                    {
                        searchData = dbp.Bytes.ToArray();
                    }
                }

                if (searchData != null && searchData.Length > 0)
                {
                    List<Socket_PacketInfo> lstFiltered = Socket_Cache.SocketList.FilterSocketList(searchData);
                    this.dgvSocketList.DataSource = lstFiltered;
                }
                else
                {
                    Socket_Operation.ShowMessageBox("请输入筛选内容");
                }
            }
            catch (Exception ex)
            {
        ...
        private void bClearFilter_Click(object sender, EventArgs e)
        {
            try
            {
                this.dgvSocketList.DataSource = Socket_Cache.SocketList.lstRecPacket;
            }
            catch (Exception ex)
            {
                Socket_Operation.DoLog(MethodBase.GetCurrentMethod().Name, ex.Message);
        ...
        private void InitSocketDGV()
        {
            try
            {
                ...
                DataGridViewTextBoxColumn cSendHotKey = new DataGridViewTextBoxColumn();
                cSendHotKey.HeaderText = "快捷键";
                cSendHotKey.DataPropertyName = "HotKey";
                cSendHotKey.Name = "cSendHotKey";
                cSendHotKey.ReadOnly = true;
                cSendHotKey.AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
                cSendHotKey.MinimumWidth = 90;
                cSendHotKey.SortMode = DataGridViewColumnSortMode.NotSortable;
                dgvSendList.Columns.Add(cSendHotKey);
                ...
                DataGridViewTextBoxColumn cRobotHotKey = new DataGridViewTextBoxColumn();
                cRobotHotKey.HeaderText = "快捷键";
                cRobotHotKey.DataPropertyName = "HotKey";
                cRobotHotKey.Name = "cRobotHotKey";
                cRobotHotKey.ReadOnly = true;
                cRobotHotKey.AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
                cRobotHotKey.MinimumWidth = 90;
                cRobotHotKey.SortMode = DataGridViewColumnSortMode.NotSortable;
                dgvRobotList.Columns.Add(cRobotHotKey);
        ...
        private void dgvSocketList_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            try
            {
                ...
                {
                    Socket_PacketInfo packetInfo = this.dgvSocketList.Rows[e.RowIndex].DataBoundItem as Socket_PacketInfo;
                    if (packetInfo == null)
                    {
                        return;
                    }

                    switch (packetInfo.FilterAction)
                    {
        ...
        private void ShowSelectSocketData()
        {
            try
            {
                ...
                {
                    Socket_PacketInfo selected = this.dgvSocketList.SelectedRows[0].DataBoundItem as Socket_PacketInfo;

                    if (selected != null)
                    {
                        Socket_Cache.SocketList.spiSelect = selected;
                        Socket_Cache.SocketList.Search_Index = Socket_Cache.SocketList.lstRecPacket.IndexOf(selected);

                        DynamicByteProvider dbp = new DynamicByteProvider(selected.PacketBuffer);
