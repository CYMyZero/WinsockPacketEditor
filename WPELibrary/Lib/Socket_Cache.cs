...
namespace WPELibrary.Lib
{
    public static class Socket_Cache
    {
        ...
        public static class System
        {
            ...
            public static Action OnHotKeyRefresh { get; set; }
            ...
            public static void LoadSystemList_FromDB()
            {
                try
                {
                    Task.Run(() =>
                    {
                        ...
                        Socket_Cache.System.InvokeAction?.Invoke(() => Socket_Cache.System.OnHotKeyRefresh?.Invoke());
                    });
                }
                catch (Exception ex)
                {
        ...
        {            
                    ...
                    if (gList.Count > 0)
                    {
                        ...
                        if (dr.Equals(DialogResult.OK))
                        {
                            foreach (Guid gAID in gList)
                            {
                                ...
                                Socket_Cache.DataBase.DeleteProxyAccount_ByGUID(gAID);
                            }

                            Socket_Cache.ProxyAccount.SaveProxyAccountList_ToDB(Socket_Cache.System.StartMode);
                        }
                    }
                }
                catch (Exception ex)
            ...
            public static void SaveProxyAccountList_ToDB(Socket_Cache.System.SystemMode FromMode)
            {
                ...
                {
                    // 读取数据库当前所有账号 GUID
                    DataTable dtDB = Socket_Cache.DataBase.SelectTable_ProxyAccount();
                    var dbGuids = new HashSet<Guid>();
                    foreach (DataRow dr in dtDB.Rows)
                    {
                        if (Guid.TryParse(dr["GUID"].ToString(), out Guid g))
                        {
                            dbGuids.Add(g);
                        }
                    }

                    // 1. 新增/编辑：内存每条 INSERT OR REPLACE（按 GUID 主键，幂等）
                    foreach (Proxy_AccountInfo pai in lstProxyAccount)
                    {
                        Socket_Cache.DataBase.InsertTable_ProxyAccount(pai);
                    }

                    // 2. 删除：DB 有、内存没有 → 删除该条
                    //    防护：内存为空且 DB 非空（加载失败/未完成）→ 跳过删除，避免清空数据
                    if (!(lstProxyAccount.Count == 0 && dbGuids.Count > 0))
                    {
                        var memGuids = new HashSet<Guid>(lstProxyAccount.Select(p => p.AID));
                        foreach (Guid g in dbGuids)
                        {
                            if (!memGuids.Contains(g))
                            {
                                Socket_Cache.DataBase.DeleteProxyAccount_ByGUID(g);
                            }
                        }
                    }
            ...
            public static void LoadProxyAccountList_FromDB()
            {
                try
                {
                    // 已加载过（内存已有数据）则跳过，避免重复加载
                    if (lstProxyAccount.Count > 0)
                    {
                        return;
                    }
                    ...
                    foreach (DataRow dataRow in dtProxyAccount.Rows)
                    {
                        ...
                        bool IsEnable = Convert.ToInt32(dataRow["IsEnable"]) == 1;
                        ...
                        bool IsLimitLinks = Convert.ToInt32(dataRow["IsLimitLinks"]) == 1;
                        int LimitLinks = Convert.ToInt32(dataRow["LimitLinks"]);
                        bool IsLimitDevices = Convert.ToInt32(dataRow["IsLimitDevices"]) == 1;
                        int LimitDevices = Convert.ToInt32(dataRow["LimitDevices"]);
                        bool IsExpiry = Convert.ToInt32(dataRow["IsExpiry"]) == 1;
        ...
        public static class ProxyMapping
        {
            ...
            public static void SaveProxyMapLocal_ToDB(Socket_Cache.System.SystemMode FromMode, bool bForce = false)
            {
                try
                {
                    if (!bForce)
                    {
                        // 防护：内存列表为空但数据库非空 → 加载失败/未完成，跳过保存避免覆盖数据
                        DataTable dtExisting = Socket_Cache.DataBase.SelectTable_ProxyMapLocal();
                        if (lstMapLocal.Count == 0 && dtExisting.Rows.Count > 0)
                        {
                            return;
                        }
                    }
            ...
            public static void SaveProxyMapRemote_ToDB(Socket_Cache.System.SystemMode FromMode, bool bForce = false)
            {
                try
                {
                    if (!bForce)
                    {
                        // 防护：内存列表为空但数据库非空 → 加载失败/未完成，跳过保存避免覆盖数据
                        DataTable dtExisting = Socket_Cache.DataBase.SelectTable_ProxyMapRemote();
                        if (lstMapRemote.Count == 0 && dtExisting.Rows.Count > 0)
                        {
                            return;
                        }
                    }
            ...
            public static void LoadProxyMapLocal_FromDB()
            {
                try
                {
                    ...
                    foreach (DataRow dataRow in dtProxyMapLocal.Rows)
                    {
                        ...
                        int Port = Convert.ToInt32(dataRow["Port"]);
            ...
            public static void LoadProxyMapRemote_FromDB()
            {
                try
                {
                    ...
                    foreach (DataRow dataRow in dtProxyMapRemote.Rows)
                    {
                        ...
                        int Port_From = Convert.ToInt32(dataRow["Port_From"]);
                        ...
                        int Port_To = Convert.ToInt32(dataRow["Port_To"]);
        ...
        public static class SocketList
        {
            ...
            public static List<Socket_PacketInfo> FilterSocketList(byte[] searchData)
            {
                List<Socket_PacketInfo> lstResult = new List<Socket_PacketInfo>();
                ...
                try
                {
                    if (searchData != null && searchData.Length > 0)
                    {
                        ...
                        {
                            byte[] packetBuffer = packetInfo.PacketBuffer;
                            if (packetBuffer != null && packetBuffer.Length >= searchData.Length)
                            {
                                if (packetBuffer.AsSpan().IndexOf(searchData) != -1)
                                {
                                    lstResult.Add(packetInfo);
                                }
                            }
                        }
                    }
        ...
        public static class Filter
        {
            ...
            #region//检查滤镜是否匹配成功（特征搜索滤镜）

            public static List<int> CheckFilter_IsMatch_Pattern(Socket_FilterInfo sfi, ReadOnlySpan<byte> bufferSpan)
            {
                var result = new List<int>();
                if (string.IsNullOrEmpty(sfi.FSearch))
                    return result;
                ...
                try
                {
                    List<byte[]> patterns = ParsePatternSearch(sfi.FSearch);
                    if (patterns.Count == 0)
                        return result;

                    bool ordered = sfi.FStartFrom == Socket_Cache.Filter.FilterStartFrom.Head;
                    int searchFrom = 0;

                    foreach (byte[] pattern in patterns)
                    {
                        int found = IndexOfPattern(bufferSpan, pattern, ordered ? searchFrom : 0);
                        if (found < 0)
                        {
                            result.Clear();
                            return result;
            ...
            private static int IndexOfPattern(ReadOnlySpan<byte> buffer, byte[] pattern, int startIndex)
            {
                if (pattern == null || pattern.Length == 0 || startIndex < 0)
                    return -1;
            ...
            private static List<byte[]> ParsePatternSearch(string search)
            {
                ...
                foreach (string part in parts)
                {
                    ...
                    foreach (string token in tokens)
                    {
                        ...
                        {
                            if (HexCharsToByte(hex.Substring(i, 2), out byte value))
                            {
                                pattern.Add(value);
                            }
                            else
            ...
            private static List<List<PatternModify>> ParsePatternModify(string modify)
            {
                var result = new List<List<PatternModify>>();
                if (string.IsNullOrEmpty(modify))
                    return result;

                string[] parts = modify.Split(';');
                foreach (string part in parts)
                {
                    var rules = new List<PatternModify>();
                    string[] ruleParts = part.Trim().Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);

                    foreach (string rule in ruleParts)
                    {
                        string[] eq = rule.Split('=');
                        if (eq.Length == 2 &&
                            int.TryParse(eq[0].Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out int offset))
                        {
                            string valueStr = eq[1].Trim();
                            if (!string.IsNullOrEmpty(valueStr) && HexCharsToByte(valueStr, out byte value))
                            {
                                rules.Add(new PatternModify { Offset = offset, Value = value });
                            }
                        }
                    }
            ...
            public static bool Replace_Pattern(Socket_FilterInfo sfi, List<int> matchPositions, Span<byte> bufferSpan)
            {
                ...
                try
                {
                    List<List<PatternModify>> allRules = ParsePatternModify(sfi.FModify);

                    for (int i = 0; i < matchPositions.Count && i < allRules.Count; i++)
                    {
                        int matchPos = matchPositions[i];
                        List<PatternModify> rules = allRules[i];

                        foreach (PatternModify rule in rules)
                        {
                            int targetIndex = matchPos + rule.Offset;
                            if (targetIndex >= 0 && targetIndex < bufferSpan.Length)
                            {
                                if (bufferSpan[targetIndex] != rule.Value)
                                {
                                    bufferSpan[targetIndex] = rule.Value;
                                    modified = true;
                                }
                            }
                        }
            ...
            public static byte[] ChangePacket_Pattern(Socket_FilterInfo sfi)
            {
                if (string.IsNullOrEmpty(sfi.FModify))
                {
                    return Array.Empty<byte>();
                ...
                try
                {
                    // 换包只用第一条数据包（第一个特征行填的包），用 ; 分隔取第一段
                    string firstPacket = sfi.FModify.Split(';')[0].Trim();
                    if (string.IsNullOrEmpty(firstPacket))
                    {
                        return Array.Empty<byte>();
                    }

                    var bytes = new List<byte>();
                    string[] tokens = firstPacket.Split(new[] { ' ', ',', '\t', '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            public static Socket_Cache.Filter.FilterAction DoFilterList(Int32 iSocket, Span<byte> bufferSpan, out byte[] bNewBuffer, Socket_Cache.SocketPacket.PacketType ptType, Socket_Cache.SocketPacket.SockAddr sAddr)
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < filters.Count; i++)
                    {
                        ...
                        else if (sfi.FMode == Filter.FilterMode.Pattern)
                        {
                            MatchIndex = Socket_Cache.Filter.CheckFilter_IsMatch_Pattern(sfi, bufferSpan);
                            isMatch = MatchIndex != null && MatchIndex.Count > 0;
                        }
                        ...
                        switch (sfi.FAction)
                        {
                            case Filter.FilterAction.Replace:
                                ...
                                else if (sfi.FMode == Filter.FilterMode.Pattern && MatchIndex != null)
                                {
                                    bDoFilter = Socket_Cache.Filter.Replace_Pattern(sfi, MatchIndex, bufferSpan);
                                    tempBuffer = bufferSpan.ToArray();
                                }
                            ...
                            case Filter.FilterAction.Change:
                                ...
                                if (sfi.FMode == Filter.FilterMode.Pattern)
                                {
                                    tempBuffer = Socket_Cache.Filter.ChangePacket_Pattern(sfi);
                                }
                                else
                                {
        ...
        public static class Robot
        {
            ...
            public static void AddRobot(bool IsEnable, Guid RID, string RName, DataTable RInstructions, string HotKey = "")
            {
                try
                {
                    if (RID != Guid.Empty && !string.IsNullOrEmpty(RName))
                    {
                        Socket_RobotInfo sri = new Socket_RobotInfo(IsEnable, RID, RName, RInstructions, HotKey);
            ...
            public static void LoadRobotList_FromDB()
            {
                try
                {
                    ...
                    foreach (DataRow dataRow in dtRobot.Rows)
                    {
                        ...
                        string HotKey = dtRobot.Columns.Contains("HotKey") ? dataRow["HotKey"].ToString() : string.Empty;
                        ...
                        Socket_Cache.Robot.AddRobot(IsEnable, RID, RName, RInstruction, HotKey);
                    }
                }
                catch (Exception ex)
                {
        ...
        public static class Send
        {
            ...
            public static void AddSend(bool IsEnable, Guid SID, string SName, bool SSystemSocket, int SLoopCNT, int SLoopINT, BindingList<Socket_PacketInfo> SCollection, string SNotes, string HotKey = "")
            {
                try
                {
                    if (SID != Guid.Empty && !string.IsNullOrEmpty(SName))
                    {
                        Socket_SendInfo ssi = new Socket_SendInfo(IsEnable, SID, SName, SSystemSocket, SLoopCNT, SLoopINT, SCollection, SNotes, HotKey);
            ...
            public static void LoadSendList_FromDB()
            {
                try
                {
                    ...
                    foreach (DataRow dataRow in dtSend.Rows)
                    {
                        ...
                        string HotKey = dtSend.Columns.Contains("HotKey") ? dataRow["HotKey"].ToString() : string.Empty;
                        BindingList<Socket_PacketInfo> SCollection = new BindingList<Socket_PacketInfo>();
                        ...
                        Socket_Cache.Send.AddSend(IsEnable, SID, SName, SSystemSocket, SLoopCNT, SLoopINT, SCollection, SNotes, HotKey);
                    }
                }
                catch (Exception ex)
                {
        ...
        {
            private static string dbPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "WPE64Cache");
            ...
            private static bool CreateTable_Send()
            {
                ...
                try
                {
                    using (SQLiteConnection conn = new SQLiteConnection(conStr))
                    {
                        ...
                        if (!ColumnExists(conn, "Send", "HotKey"))
                        {
                            using (SQLiteCommand cmdAlter = new SQLiteCommand("ALTER TABLE Send ADD COLUMN HotKey TEXT;", conn))
                            {
                ...
                try
                {
                    using (SQLiteConnection conn = new SQLiteConnection(conStr))
                    {
                        ...
                        using (SQLiteCommand cmd = new SQLiteCommand(sql, conn))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@HotKey", ssi.HotKey);
                            cmd.ExecuteNonQuery();
                        }
            ...
            private static bool CreateTable_Robot()
            {
                ...
                try
                {
                    using (SQLiteConnection conn = new SQLiteConnection(conStr))
                    {
                        ...
                        if (!ColumnExists(conn, "Robot", "HotKey"))
                        {
                            using (SQLiteCommand cmdAlter = new SQLiteCommand("ALTER TABLE Robot ADD COLUMN HotKey TEXT;", conn))
                            {
                                cmdAlter.ExecuteNonQuery();
                            }
                        }
                ...
                try
                {
                    using (SQLiteConnection conn = new SQLiteConnection(conStr))
                    {
                        ...
                        using (SQLiteCommand cmd = new SQLiteCommand(sql, conn))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@HotKey", sri.HotKey);
                            cmd.ExecuteNonQuery();
                        }
            ...
            public static void DeleteProxyAccount_ByGUID(Guid guid)
            {
                try
                {
                    using (SQLiteConnection conn = new SQLiteConnection(conStr))
                    {
                        string sql = "DELETE FROM ProxyAccount WHERE GUID = @GUID;";
            ...
            public static void InsertTable_ProxyAccount(Proxy_AccountInfo pai)
            {
                try
                {
                    using (SQLiteConnection conn = new SQLiteConnection(Socket_Cache.DataBase.conStr))
                    {
                        string sql = "INSERT OR REPLACE INTO ProxyAccount (" +
                                     "GUID, IsEnable, UserName, PassWord, LoginTime, LoginIP, IPLocation, IsLimitLinks, LimitLinks, IsLimitDevices, LimitDevices, IsExpiry, ExpiryTime, CreateTime" +
                                     ") VALUES (" +
                                     "@GUID, @IsEnable, @UserName, @PassWord, @LoginTime, @LoginIP, @IPLocation, @IsLimitLinks, @LimitLinks, @IsLimitDevices, @LimitDevices, @IsExpiry, @ExpiryTime, @CreateTime" +
                                     ");";
                        ...
                        using (SQLiteCommand cmd = new SQLiteCommand(sql, conn))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@IsLimitLinks", pai.IsLimitLinks);
                            cmd.Parameters.AddWithValue("@LimitLinks", pai.LimitLinks);
                            cmd.Parameters.AddWithValue("@IsLimitDevices", pai.IsLimitDevices);
                            cmd.Parameters.AddWithValue("@LimitDevices", pai.LimitDevices);
