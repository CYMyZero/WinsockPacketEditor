...
namespace WPELibrary.Lib.UserControl
{
    public class HotkeyTextBox : TextBox
    {
        ...
        public static bool RegisterHotkeyText(IntPtr handle, int keyId, string hotkeyText)
        {
            try
            {
                if (string.IsNullOrEmpty(hotkeyText) || handle == IntPtr.Zero)
                {
                    return false;
                }

                Keys key = ParseHotkeyString(hotkeyText);
                if (key == Keys.None)
                {
                    return false;
                }

                User32.UnregisterHotKey(handle, keyId);
                ...
                uint vk = (key >= Keys.NumPad0 && key <= Keys.NumPad9)
                    ? (uint)(key - Keys.NumPad0 + 0x60)
                    : (uint)(key & Keys.KeyCode);

                return User32.RegisterHotKey(handle, keyId, modifiers, vk);
            }
            catch (Exception ex)
            {
                Socket_Operation.DoLog(MethodBase.GetCurrentMethod().Name, ex.Message);
